...
입력 데이터 해석 규칙:
- 집계는 제품군별(skincare/makeup)로 분리되어 있고, 플랫폼 가중치가 반영된 점수다
  (스킨케어: 올리브영·화해·무신사뷰티 / 색조: 올리브영·무신사뷰티·지그재그).
- cross_platform_consensus는 2개 이상 플랫폼에서 동시에 뜨는 키워드 — 가장 신뢰할 신호.
- diffusion의 early(지그재그·무신사 선행)는 색조 신제품의 적기, diffusing은 스킨케어까지
  포함해 지금 진입할 기회, mainstream(올리브영 단독)은 이미 성숙해 차별화 없이는 늦은 시장,
  watch(화해 등 관심 신호만 존재)는 성분·효능 관심은 확인됐으나 판매로 이어지는지 추적할 후보.
