...
상품 관리 구글시트의 상품별 도매처 페이지를 주기적으로 확인해서, **현재공급가·품절여부·확인시각을 시트에 자동 기록**하고
변경(가격 인상/인하, 품절 발생/해제, 시트 기준 공급가 초과)이 있으면 텔레그램으로 알립니다.

설정 순서:

1. **서비스 계정 만들기** — [Google Cloud Console](https://console.cloud.google.com) > 프로젝트 생성 >
   "Google Sheets API" 사용 설정 > 서비스 계정 생성 > JSON 키 다운로드 → `service_account.json` 으로 저장
2. **시트 공유** — 구글시트 공유 버튼 > 서비스 계정 이메일(`...@...iam.gserviceaccount.com`)을 **편집자**로 추가
3. **시트에 `공급처URL` 열 추가** — 헤더 행에 `공급처URL` 열을 만들고, 상품별로 도매처 상품페이지 주소를 붙여넣기
   (이 열이 채워진 행만 감시 대상이 됩니다. `현재공급가`/`품절여부`/`확인시각` 열은 자동 생성됩니다)
4. **config.yaml** 의 `google_sheet` 섹션에 스프레드시트 ID·탭 이름·JSON 경로 입력
5. **로그인 필요한 몰**(대부분의 B2B 도매몰) — 공급처 `watch.fetch: playwright` 설정 후 1회 로그인:
   ```bash
   pip install playwright && playwright install chromium
   coupang-auto watch-login woleokdojeon https://도매처로그인주소
   ```
   열리는 브라우저에서 로그인하고 터미널에서 Enter — 세션이 저장되어 이후 자동 사용됩니다.

실행:

```bash
coupang-auto watch-run    # 즉시 1회 확인 (설정 검증용)
coupang-auto run-daemon   # 데몬이 watch.interval_hours(기본 6시간)마다 자동 확인
```

가격 파싱이 안 맞는 몰은 `config.yaml` 공급처의 `watch.price_selector` / `watch.soldout_selector` 에
CSS 셀렉터를 지정하면 됩니다. 도매처 로그인 세션이 만료되면 "확인실패"로 표시되니 `watch-login` 을 다시 실행하세요.
...
- [x] **가격·품절 감시**: 구글시트 × 도매처 사이트 연동 (`watch-run` / 데몬 주기 실행)
- [ ] **2주차 — 송장 자동화**: 공급처 송장 회수(Playwright/메일) → 쿠팡 송장 업로드(`upload_invoice` 이미 준비됨) + 발주 후 24시간 송장 미등록 알림
- [ ] **3주차 — 오픈채팅 품절·가격 감시**: 공기계 알림 포워딩 → LLM 파싱 → 재고 0 처리/가격 조정 제안 (사이트 감시와 상호 보완)
- [ ] **가격 감시 → 쿠팡 자동 조치**: 품절 시 쿠팡 재고 0 처리, 공급가 인상 시 판매가 자동 조정 (현재는 알림까지)
